   ...
   return new Promise((resolve, reject) => {
      fetch(
         ...
         .then((res) => {
            ...
            data.splice(2, 1);
            if (data.length == 0)
               return resolve({ creator: "© AdrianTzy", status: false });
...
async function igstory2(username) {
   let { data } = await axios.get(
      `https://www.instadownloader.org/data.php?username=${username}&t=${
         new Date() * 1
      }`
   );
   const $ = cheerio.load(data);
...
function facebook2(urls) {
   return new Promise(async (resolve, reject) => {
      ...
      const getPost = await axios({
         ...
         headers: {
            "user-agent":
               "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/67.0.3396.99 Safari/537.36",
            cookie:
               "_gid=GA1.2.1321544464.1633811193; _ga=GA1.2.1392580783.1633811193; __gads=ID=c73de99d7fa5c467-226981f63ecc00f1:T=1633811193:RT=1633811193:S=ALNI_MaC9fW2mqfT2hm7zODcNNffab1XLg",
         },
      ...
      const hasil = {
         ...
         thumb: c("div.row")
            .find("div.col-md-12:nth-child(2) > img")
            .attr("src"),
         link_high:
            c("div.row").find("div.col-md-3 > a").eq(0).attr("href") ||
            c("div.row").find("#sd > a").attr("href") ||
            "",
         link_normal:
            c("div.row").find("div.col-md-3 > a").eq(1).attr("href") || "",
      };
   ...
   return new Promise((resolve, reject) => {
      ...
      Axios.get("https://mobile-legends.fandom.com/wiki/" + que)
         .then(({ data }) => {
            ...
            let skill = [];
            const name = $(
               "#mw-content-text > div > div > div > div > div > div > table > tbody > tr > td > table > tbody > tr > td > font > b"
            ).text();
            $(".mw-headline")
               .get()
               .map((res) => {
            ...
            $(
               "#mw-content-text > div > div > div > div > div > div > table > tbody > tr > td"
            )
               .get()
               .map((rest) => {
            ...
            $(
               "#mw-content-text > div > div > div > div > div > div > table > tbody > tr > td > div.progressbar-small.progressbar > div"
            )
               .get()
               .map((rest) => {
            ...
            Axios.get(
               "https://mobile-legends.fandom.com/wiki/" + que + "/Story"
            )
               .then(({ data }) => {
                  const $ = cheerio.load(data);
                  let pre = [];
                  ...
                  const img = $(
                     "#mw-content-text > div > aside > figure > a"
                  ).attr("href");
                  const chara = character.slice(0, 2);
                  const result = {
                     status: 200,
                     hero_name: name + ` ( ${mw[0].replace("CV:", " CV:")} )`,
                     entrance_quotes: attrib[2]
                        .replace("Entrance Quotes", "")
                        .replace("\n", ""),
                     hero_feature: attrib[attrib.length - 1].replace(
                        "Hero Feature",
                        ""
                     ),
                     image: img,
                     ...
                     attributes: {
                        movement_speed: attrib[12].replace(
                           "● Movement Speed",
                           ""
                        ),
                        physical_attack: attrib[13].replace(
                           "● Physical Attack",
                           ""
                        ),
                        magic_power: attrib[14].replace("● Magic Power", ""),
                        attack_speed: attrib[15].replace("● Attack Speed", ""),
                        physical_defense: attrib[16].replace(
                           "● Physical Defense",
                           ""
                        ),
                        magic_defense: attrib[17].replace(
                           "● Magic Defense",
                           ""
                        ),
                        basic_atk_crit_rate: attrib[18].replace(
                           "● Basic ATK Crit Rate",
                           ""
                        ),
                        hp: attrib[19].replace("● HP", ""),
                        mana: attrib[20].replace("● Mana", ""),
                        ability_crit_rate: attrib[21].replace(
                           "● Ability Crit Rate",
                           ""
                        ),
                     ...
                     price: {
                        ...
                        hero_fragment: mw[1].split("|")[2]
                           ? mw[1].split("|")[2].replace(/ /g, "")
                           : "none",
                     },
                     role: mw[2],
                     skill: {
   ...
   return new Promise((resolve, reject) => {
      axios
         ...
         .then(({ data }) => {
            const hasil = [];
            const $ = cheerio.load(data);
            $("#siteContainer > ul.cardDeck.cardGrid > li ").each(function (
               a,
               b
            ) {
               result = {
            ...
            $("#main_mid_headline_sub_topic").each(function (a, b) {
               result = {
                  ...
                  tanggal: $(b)
                     ...
                     .split("                            ")[1],
                  jam: $(b)
                     .find(" > div.main_mid_headline_topic > span")
                     .text(),
                  url: $(b)
                     .find(" > div.main_mid_headline_topic > div > a")
                     .attr("href"),
         ...
         .then((data) => {
            const random =
               data.data.resource_response.data.results[
                  Math.floor(
                     Math.random() *
                        data.data.resource_response.data.results.length
                  )
               ];
            var result = [];
            result = {
   ...
   return new Promise((resolve, reject) => {
      axios
         ...
         .then(({ data }) => {
            const $ = cheerio.load(data);
            const hasil = [];
            $("div.story-card-data.hidden-xxs > div.story-info ").each(
               function (a, b) {
         ...
         .then(({ data }) => {
            ...
            $(
               ...
               result = {
                  ...
                  link:
                     "https://mangatoon.mobi" + $(b).find("> a").attr("href"),
   ...
   return new Promise((resolve, reject) => {
      axios
         ...
         .then(({ data }) => {
            const $ = cheerio.load(data);
            const hasil = [];
            $("body > div:nth-child(4) > div > div.col-md-6 > div ").each(
               function (a, b) {
                  result = {
                     status: 200,
                     author: "© AdrianTzy",
                     surah: $(b)
                        ...
                        .text(),
                     tafsir: $(b)
                        .find("> div.panel-body.excerpt")
                        .text()
                        .trim(),
                     type: $(b)
                        .find(
                           "> div.panel-heading.panel-choco > div > div > span"
                        )
                        .text(),
                     source: $(b)
                        .find("> div.panel-heading.panel-choco > div > div > a")
         ...
         .then((tod) => {
            ...
            $("li.p1520.art-list.pos_rel").each(function (c, d) {
               ...
               desc = $(d)
                  ...
                  .trim();
               date = $(d)
                  .find("div.grey.pt5 > time.foot.timeago")
                  .text()
                  .trim();
            ...
            $(
               ...
               result = {
                  ...
                  type_berita: $(b)
                     ...
                     .text(),
                  link: $(b)
                     .find("> div > div.article__box > h3 > a")
                     .attr("href"),
                  thumbnail: $(b)
         ...
         .then(({ data }) => {
            ...
            $("#news-list > li ").each(function (a, b) {
               result = {
                  ...
                  berita: $(b)
                     .find(
                        "> a > div > div > div.float-left.width-400px.margin-130px-left > h3"
                     )
                     .text()
                     .trim(),
                  upload_time: $(b)
                     .find(
                        "> a > div > div > div.float-left.width-400px.margin-130px-left > div.date.margin-10px-left"
                     )
                     .text()
                     .trim()
                     .split("|")[0],
                  ...
                  thumbnail: $(b)
                     .find(
                        "> a > div > div > div.float-left.width-130px.position-absolute > img"
                     )
                     .attr("data-original"),
                  info_berita: $(b)
                     .find(
                        "> a > div > div > div.float-left.width-400px.margin-130px-left > p"
                     )
                     .text(),
               };
               hasil.push(result);
   ...
   return new Promise(async (resolve, reject) => {
      axios
         ...
         .then(({ data }) => {
            const $ = cheerio.load(data);
            const hasil = [];
            $("#case > div > div > div > div > div:nth-child(2)").each(
               function (a, b) {
                  ...
                  const upindo = $(b)
                     ...
                     .text()
                     .trim();
                  $("#case > div > div > div > div > div:nth-child(1)").each(
                     function (c, d) {
                        const neg = $(d)
                           .find("div:nth-child(3) > strong")
                           .text();
                        const pglo = $(d)
                           .find("div:nth-child(4) > strong")
                           .text();
                        const nglo = $(d)
                           .find("div:nth-child(5) > strong")
                           .text();
                        const up = $(d)
...
function servermc() {
   return new Promise((resolve, reject) => {
      axios
         .get(`https://minecraftpocket-servers.com/country/indonesia/`)
         .then((tod) => {
            const $ = cheerio.load(tod.data);
            hasil = [];
...
function ssweb(url) {
   return new Promise(async (resolve, reject) => {
      await getBuffer(
         `http://fdz-app.herokuapp.com/api/ssweb/mobile/get_screenshot?url=${url}`
      )
         .then((res) => {
            resolve(res);
         })
